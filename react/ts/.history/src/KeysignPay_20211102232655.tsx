...
> = ({
  ...
  const [isKeysignInstalled, setIsKeysignInstalled] = useState(false);
  ...
  const waitForKeysign = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (keysignObj || iterations >= 3) {
        ...
        if (keysignObj) {
          setIsKeysignInstalled(true);
        } else {
          setIsKeysignInstalled(false);
        }
        ...
        <div className="w-full px-5 h-full flex gap-5 flex-col items-center">
          ...
          {useKeysign && (
            <div className="flex flex-col w-full gap-1">
              <button
                disabled={!isKeysignInstalled}
                className={`${
                  isKeysignInstalled ? "app-button" : "disabled-button"
                }`}
                onClick={() => {
